...
def review_single_order(order: dict, signal: dict) -> dict:
    """对单笔下单决策做 AI 复审，返回判定 dict。"""
    resp = Generation.call(
        model=config.ORCHESTRATOR_MODEL,
        messages=[
            {"role": "system", "content": _REVIEW_VERDICT_SYSTEM},
...
def review_orders(orders: list[dict], progress=None) -> dict:
