...
def _lessons_block(lessons: str) -> str:
    """把用户交易铁律拼成 system 追加段(软约束,量化基底优先)。"""
    lessons = (lessons or "").strip()
    if not lessons:
        return ""
    return ("\n\n【用户个人交易铁律 —— 在不违反量化纪律的前提下务必遵守】\n"
            + lessons
            + "\n(以上为用户长期经验,作为方案的软约束;若与量化数据强烈冲突,以量化为先并在 risk_note 说明。)")
...
def run_portfolio(
    ...
    call_kwargs = dict(
        model=config.ORCHESTRATOR_MODEL,
        messages=[
            {"role": "system", "content": _build_portfolio_system(use_research, use_sentiment) + _lessons_block(lessons)},
...
_REVIEW_VERDICT_SYSTEM = """你是一位 A股交易复审主管。下方给出"今日统筹方案的一笔交易"以及"经过多Agent多空博弈得到的该股交易信号"。请对这笔交易给出复审判定。

**只输出一个 JSON 对象**(不要任何解释、不要围栏)：
{
  "verdict": "合理/有保留/反对 三选一",
  "confidence": "高/中/低",
  "agree_points": "博弈结论与下单方向一致之处(一两句)",
  "concerns": "博弈结论中与下单方向相左/需注意的点(一两句)",
  "suggested_price": 数字或null,    // 如反对原下单价，给出更稳妥的建议触发价
  "suggested_lots": 数字或null,     // 如建议调整数量(整手)
  "stop_loss": 数字或null,          // 该笔交易的建议止损价
  "take_profit": 数字或null         // 该笔交易的建议止盈价
}
判定标准：
- 合理：博弈与下单方向一致、关键风控条件满足
- 有保留：方向一致但有具体风险条件需关注(如压力位、估值、止损偏松)
- 反对：博弈结论与下单方向矛盾，或博弈给出明确反向信号
"""


def review_single_order(order: dict, signal: dict, lessons: str = "") -> dict:
    """对单笔下单决策做 AI 复审，返回判定 dict。"""
    resp = Generation.call(
        model=config.ORCHESTRATOR_MODEL,
        messages=[
            {"role": "system", "content": _REVIEW_VERDICT_SYSTEM + _lessons_block(lessons)},
            {"role": "user", "content": json.dumps({
                "下单决策": {k: order.get(k) for k in
                          ("symbol", "name", "side", "action", "shares", "price", "amount", "note")},
                "多Agent博弈信号": signal,
            }, ensure_ascii=False)},
        ],
        api_key=config.API_KEY,
    ...
    if resp.status_code != 200:
        raise Exception(f"复审 API 错误 {resp.status_code}: {resp.message}")
    return _strip_json(resp.output.text)


def review_orders(orders: list[dict], progress=None, lessons: str = "") -> dict:
    """对下单清单逐单复审：每只股先做多Agent博弈拿到独立信号，再由复审主管判定是否合理。"""
    def emit(m):
        if progress:
            progress(m)

    # 去重：同一股(symbol)可能出现多个 leg(止盈+止损),只博弈一次,复审复用
    ...
    for i, sym in enumerate(syms, 1):
        nm = by_sym[sym][0].get("name") or sym
        emit(f"[{i}/{len(syms)}] 多Agent博弈 + AI 复审：{nm}…")
        try:
            sig = run_trading_signal(sym, nm)
        except Exception as e:
            sig = {"error": str(e), "view": f"博弈失败：{e}"}
        for o in by_sym[sym]:
            try:
                v = review_single_order(o, sig, lessons)
            except Exception as e:
                v = {"verdict": "—", "concerns": f"复审失败：{e}"}
            reviews.append({
                "symbol": sym, "name": nm,
                "side": o.get("side"), "action": o.get("action"),
                "shares": o.get("shares"), "price": o.get("price"),
                "amount": o.get("amount"), "original_note": o.get("note"),
                "signal": sig,
                "review": v,
            })
